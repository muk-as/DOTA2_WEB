// 8644.js

/**** (c) Valve Corporation. Use is governed by the terms of the Steam Subscriber Agreement http://store.steampowered.com/subscriber_agreement/.
 ****/
(self.webpackChunkdota_react = self.webpackChunkdota_react || []).push([
  [8644],
  {
    85655: (e) => {
      e.exports = {
        Tooltip: "_19T9N3UHHbAODE9sxxoLtb",
        CarouselFade: "_D1I4Yzr_NSlA70n0WMzr",
        StandardButton: "mgvs9cPEYOJ3Q9r1J6Xvv",
        ButtonText: "_2MUkIuEbGuL6NdEm21Eoz9",
        Icon: "_2dDXk1s2V92ElJmDhv4kOC",
        Play: "_1ZhvqFTdNClE1GSKe9lIwC",
        SteamLogo: "_2GSNhjmGVrkHS1HRhAYMiS",
        ToolTip: "S2816E7lw3xwpK0fpEU9j",
        PlayerReportTooltip: "_3UgI1sTrl6KyLY4GdZpwRz",
        ComparisonImage: "_3SX3hhAHRpb7bRPg9YLyR2",
        ImageLabel: "_3bKKQTvBpbxJ08XrDQJ4xn",
        IsNew: "S0b_nhi5aY46bpyCNasDF",
        TabbedMapComparison: "_3o6_Ks0vSIsM1b7E9OlHj",
        TabHeader: "_2kgUPa_RFGPVkckZE2a7ab",
        Tab: "_1wY4IE4Lg3TreYGTu1DSxd",
        Active: "ZBQE3qlmuiEadHG7T6gCw",
        TabContents: "_15E7RnjZvLS7TPcNOg_MN8",
        TabContentContainer: "_2tKRoICKxcJBUHYRH5k9JZ",
      };
    },
    13837: (e) => {
      e.exports = {
        Tooltip: "Fx-uDHxSYrhU7sUg5BaTa",
        CarouselFade: "Rzy5AZOuCpdfH_J53qh67",
        StandardButton: "_2MMe03tv2eGzSU_56VNdwC",
        ButtonText: "_6hj-5GY-ZYpwK-dkkad3t",
        Icon: "_1B6-U65kWofgKu3vrmMA6V",
        Play: "gzox2PyXPi-g-xGssaE-s",
        SteamLogo: "_5f4KMuXaFJ2aBt78V6dbX",
        ToolTip: "_3w2JFqkaHjsCJVnSZzMNF4",
        PlayerReportTooltip: "_1T2vso-IndU_kZL3l8G7ZJ",
        Facet: "_2HcPTYK1pLftmdMdeH7int",
        FacetColorRed0: "_1SSNLQA4EDZhhUwKlynhjX",
        BackgroundTexture: "_12QeGzD9q64QUnUo1ix_oh",
        FacetColorRed1: "_2ouOrKvZIAYgHjgcbf7e3m",
        FacetColorRed2: "_39d643kMV8S3-ivTdIfMWW",
        Background: "_1SEg41sTeDAIjTMBfY3Ntu",
        FacetColorYellow0: "mw-z4zPWqwZn9jpGbqGuX",
        FacetColorYellow1: "_14qOBcN8LRedY3gzBal2KZ",
        FacetColorYellow2: "W3eYS9TTVOPD42DrgRODO",
        FacetColorYellow3: "_6UYAz2WtWHU4_F7rhU_Jh",
        FacetColorGreen0: "i1wPcNA3Iom35vG7R7n_l",
        FacetColorGreen1: "_27cHAW0w57_V1n2-zk8I4Z",
        FacetColorGreen2: "_1hCmdMRRGU4vRrlJSWjDbj",
        FacetColorGreen3: "_1ewgq5r__SksDZG414DsZ3",
        FacetColorGreen4: "_2muCdUWF3gKiDgqxCPwByL",
        FacetColorBlue0: "_2b2zadjcEPEb7zdXYzvBFf",
        FacetColorBlue1: "_2AHW4nSqYXV8aVOBDXlIhw",
        FacetColorBlue2: "_3cX6AgFa78xlFoj0i32XOT",
        FacetColorBlue3: "_1yC70MtEbZ9NqGWXRwbdPW",
        FacetColorPurple0: "_1BQW-oAAGhJcIB5aN7BJgi",
        FacetColorPurple1: "_2s3NaEhnqLV2Ai3z_tdPa0",
        FacetColorPurple2: "_1Y6nt9r85XfK8ylRz6DP3m",
        FacetColorGray0: "_2aeXdH3lV15LkRVAdM21xw",
        FacetColorGray1: "_3VOPSvIg_HMhn7_85Bn-Yl",
        FacetColorGray2: "_1aDkkMJ72UbjD7KMVt_Mmg",
        FacetColorGray3: "RNu52qP5W7ZE6KNzElxmk",
        HeroHighlight: "_14j_dBntS0skxqDQl6JIHZ",
        HeroHighlightBorder: "_2_bFjwnCy94UB8ESW6h02_",
        HeroHighlightVideoContainer: "wgZxK5Mau7_fqsk8MRvpI",
        HeroHighlightFloorShadow: "_38fuvlzY60potYq4-1j8gN",
        HeroHighlightVideo: "_3sxZrdfcwu3iUjBsWdaxzf",
        FacetsContainer: "_3Ccsd4jD0cWZKSL29sma6h",
        HeroNameInfoContainer: "_3IshIWHHu5pF1KyWCWw4R-",
        HeroName: "_2Dt9D8RLXUFdyCx7j5AtGw",
        HighlightTitle: "_1DEJjNhomN2wUmYc1yi0aC",
        NewBorder: "_1xWynSiVO_31VJ6dQ_Yvgi",
        NewBadge: "_1W63911_vFtDnxfUaSeQ52",
        ReworkedBorder: "gfkPyjbhENxeijOw6CWyO",
        ReworkedBadge: "_2yYZg4dQUtbtlk57zc4IZy",
        NewFacet: "_3i4M2u2xF5mpf_gWpVb0Mu",
        ReworkedFacet: "_3Dvmo3jVH20wDuri42JtmN",
        FacetHeader: "_3olLxif89i5XShneDQr30m",
        HeaderContents: "pzn6Ka3F0z4J4vN_3yuBJ",
        IconContainer: "_2djFc86ivnduBfmQ1-v_Bm",
        IconBackground: "_2WcLXloyQqbS9E2FzJDUMn",
        IconWash: "djfgHhWm7XDq8gtizN8EK",
        Name: "_3FGHET_UhQcHCQJQSxXY5g",
        FacetBodyFiller: "_3xJpTjVaEM3a2n8Vq_JPEb",
        FacetNotes: "_3U3PM5fb2QMDP2h391ToET",
        FacetNote: "_1rU6dqtBRI5MX3aaSWEOep",
        Dot: "_1-Xw5jD8pIQWXt8LfyZyJ4",
        IsHidden: "SBusVCAWWInsR2iIPgyGF",
        FacetAbilities: "_3SCWwzTBt13zBzM87raEPo",
        FacetAbility: "_1fziltzTM5pJQelkUGfOk-",
        AbilityIcon: "_3fQCAlH1TM02ofIpQVM6M3",
        RightSide: "_1FyT6Tamh9R4Er6Xj3U0WC",
        AbilityName: "_24PyFaa-LjT9Zi22j6DqW6",
        AbilityNote: "_2Mjym6KRIwuoDRcUc5rV0f",
        FacetTalentNotes: "_1_RKX-o378pXZ_poj92zBV",
        TalentImage: "_39shVdNj0_7qNNABrp4ubc",
        NoteElement: "_24hmCy5Tx3Xhoaq-320uv9",
        Note: "_4KurQbwTz3ndBBdQl9bzc",
      };
    },
    26942: (e) => {
      e.exports = {
        Tooltip: "_3zf-ziwgz2rcHjk1c5cqne",
        CarouselFade: "_28KWLguZzXjTGGFfz_gO-t",
        StandardButton: "lmVbA57qNDgAp5l53bv8c",
        ButtonText: "_3jqlDlfvvY61aOW-QqKkxO",
        Icon: "_15pkl_ifEp1T78Ygu4j94h",
        Play: "_21I4lYW5k68DyAYsDQsGAj",
        SteamLogo: "_1trtyG4RWYNvLGs3ufnxNR",
        ToolTip: "_22kpKfoADDG261HLF1Q-_H",
        PlayerReportTooltip: "i7fMUYnChafEExl6NyDPw",
        LightGrayText: "_25b4968l564w1aKgzuRp9w",
        GrayText: "_1f1qEoJCkSFKH7Xj14cYZ5",
        TitleFont: "_2u5KnD3wmD62UzBtQp17pE",
        TitleExtraLarge: "-UQCPn23i8xJBxizWxKBC",
        TitleLarge: "OrFBXkqcpkUMmsI09TonK",
        TitleMedium: "_32sEiP8mQJtKMTOtf_uUN0",
        TitleSmall: "j3iMgmtEaYRNCzNboBieI",
        TitleExtraSmall: "kREyv_maSm8yS2K1lI5x0",
        DisplayFont: "_1c4ZT7UXV0PJC0PWhd0CWk",
        DisplayExtraLarge: "_1qTqi7SzzimAZYgzm3rTQG",
        DisplayLarge: "XakfDOwYZw9Z971bYUCux",
        DisplayMedium: "_2XfSRnvbP4Kr7dn2_cD5gi",
        DisplaySmall: "VTdaSiwbr21h1AkNW8C9e",
        BodyFont: "ygDv3TYULhdanwMBbomI2",
        BodyExtraLarge: "xfSYWFCattT8eEhBgWzUx",
        BodyLarge: "_18vIqb2Rw6SMkhXolZ21rX",
        BodyMedium: "_2ZIgFwAf4aEws9n1eUOTHz",
        BodySmall: "b-DBbJUPiIV_TBg30-Nlc",
        LabelFont: "_1rz1Hgjdr4VON87M9do5s3",
        LabelExtraLarge: "Fz9pOCvjvRluEJqLVPsJn",
        LabelLarge: "_10GZok66s9AYv27Ph8s06y",
        LabelMedium: "_1H6PgymyCIM19T5ABDyF2h",
        LabelSmall: "_2z5EtM7yLarKrDpVvlVhTy",
        ControlIcon: "_2LAwqqe1tjDBhvqbBFp-f9",
        SectionBackgroundImage: "_2cfD-IuDYBLgEzZZWMvMue",
        ImageShadowMedium: "_3RqGLjQr00TKvRozRQ-PTW",
        ImageShadowSmall: "_3vPe54VNAiLtJv55jZBPb5",
        PageContainer: "HC_MZYQwVIDG5wy5vgn_4",
        Hidden: "_27dRCFArSyC6aHmrJLvDeq",
        Springcleaning2025: "beOjDjD9c13jcSbLN8lIY",
        MobileOnly: "_3jjaumYBEdUU0q4aasTosj",
        WebsiteSection: "dcIHySIMcOm0VhIv8u2GG",
        WebsiteSectionInner: "_3k0-_dfGiKpDf6DZu_1ChJ",
        InnerContainer: "_3VyGDo3SfclJ5Cds1jYxym",
        WebsiteSectionHeader: "_1B6g0WsbOrkxky0WQwU057",
        TextSection: "_1UCREzJZRKqVMbwOeB4Qu_",
        SubsectionDivider: "h-9DtHVxdbMZvFxuoDmMa",
        TopDash: "nq8BykjQoTu83N_XHDOpm",
        Background: "_1qGOPupBRi04bhcsIUbjgn",
        SectionDivider: "_1UlbbaoZNW96PsiW9M7V_E",
        Pattern: "_1oHHIrr_HvXSlW5r_yLpTt",
        Overlay: "U8bkh-sBehUVKjAE9tx_U",
        BottomDash: "_3HtSLeC-qRdNokjZ04D4-8",
        ButtonsSection: "_3toCp38lICmYPcp_bKzOjU",
        Grid_4: "_2gDrSoOSTX7-R-NI7m9_Cx",
        Grid_3: "if3SQEmEMP23a_KEmXYFx",
        Grid_2: "_2SeAYdCSODopZM4gHwcOT-",
        PingsGrid: "EFMOFGJHbkB0xEwsTGp_o",
        TextImageBlockVertical: "_2kAxU-4tzaBnhCZE1TA06k",
        TextBlock: "_2S3PHDZgXFpHPlK2q0BAaK",
        TextImageBlockHorizontal: "_3xskdSIJE_PmijVmkH1Ohy",
        Flipped: "_3XxcVyacSMmDGmqC46jtX2",
        NarrowImage: "_2M56O1IODcUVAlPxfRThaU",
        WideImage: "_3M4aZBRLsqWhcoauvrlw7C",
        FullWidthImage: "_1RxFrziYgJZSH1Nss0_mCc",
        ActionTextImageBlock: "_17TIgBMRoinBHV3AEDWmVj",
        ActionImage: "LLrtYGAdb_Z66yIiCroFy",
        HeroReworkContainer: "-NLnQXXW5DkK87UkOiUx8",
        HeroRework: "_1x8LUifru6KmUTO0b0UuLn",
        HeroName: "ec-dbLu0ZX4iAypxcUKE4",
        ReworkDescription: "_1ynJPnzJMlvkO09LmsrKxn",
        HeroImageContainer: "_3kNCCe-9gxpt2ABWRq8xB1",
        HeroShadow: "_3FXpNWXaSRaL8CinLADowp",
        HeroReworkPortrait: "Z9nj8XE2ZpmWF920m3IqM",
        HeroReworkPortraitVideo: "QndJgMVXHfiKESFx2rtOJ",
        HeroReworkPatchNotes: "_2I02Opei6r-4YnMc_YFIl9",
        Muerta: "_2Y__fa31PD2uftri-qdP7I",
        Clinkz: "_1uIsLf0qYZfQgSWMLF7YaK",
        ArcWarden: "_3fgmXQTf95mqw2XcggyPYD",
        ImmersiveTextImageBlock: "_TyvL-UofaHezS-WFVp1E",
        DashedSectionSubHeader: "_2uyHCdUZfWdkTPv4Aiu6gH",
        Label: "rwPSyoNBZc767Wca9IIpe",
        DashLeft: "JSgCmEJn32SODL1j-2VP7",
        DashRight: "_1IA4ub5GBiQtoyGFB4Bmuk",
        AbilityImageContainer: "_3SgaGNENZY91haXpYDsRdE",
        AbilityImage: "_2W6DMxs3nhEDRoJXuwhhst",
        AbilityHotKey: "_3Q1SrXf9A0YtSfsqotkaeq",
        Active: "MYmyIEjDz0udNyUaGWxvY",
        DotaPlusBadge: "_3n0loZKZmEJghssPseJ25X",
        HeaderSection: "zFJHUd9D_nk-ChbfuhgXz",
        BackgroundGradient: "liVHnvWClbHMwejWXxQ88",
        HeaderHeroImage: "_5lCRXwI2a0AhTRqCqJlZN",
        HeaderFeatureImage: "_1qvqB3WNJUqev4KNFE2Z0X",
        HeaderTextSection: "_3Oa_MJMYkSnwiA2nOo6eWT",
        WebsiteIntro: "_38DrmsyqNCTMojg2h8gUXP",
        TreasureSection: "_38P6aQeLEBE5J00FIx0RGz",
        TreasureBackground: "_1u0NG_8EuU3ELFiT6W3KV",
        TopShadow: "_1aBxbEEuJSnwQ5KxIeKH9t",
        TreasureCarousel: "_2nverfs8Hl5OBII4Wy4Bnn",
        TreasureSlider: "_3dJw5gHPYFTfJkNZhJSjbH",
        TreasureSlide: "_1oe8Zg4Zudko_2_xptgeER",
        SlideContainer: "_3Wo8LbEWu87k5dzbjO28qZ",
        TreasureSlideHidden: "_3FFeCWoYcZ_FTPMGRepeY-",
        TreasureName: "lCYD0X3XgSB0Z7AGxAvfQ",
        CarouselDots: "_3Kft36oavyKPZ3fPlbcjfP",
        TreasureSelector: "JsgG8AhyB36Xi69U1HlFZ",
        TreasurePaginationButton: "_3ukunJLKZZYdR8vHFbyEXv",
        Prev: "_3fN-T_2UsHHH02mIqwOr4k",
        Next: "_1zRsHsSa-KkUBV2KA5CaWH",
        NextArrow: "_3O8kmKYesGA9fO8D3hDHYt",
        PrevArrow: "_3Ny-Ttxlr6cUmVCE9L_IuX",
        GameplayUpdateContainer: "_1ut6v5cgWBazycQqNoNk3h",
        PatchnotesContainer: "hh1nc48kX0ciM4zeMDdEW",
        PatchNotesHeaderLabel: "_1RQyF2eMiRgiryxlbR7Kos",
        MiscSection: "R9bEwz8Bp2dKtnj4-ooEl",
        UpdatesSection: "_1m4EfOTm57VMgOUD_8di9C",
        GameItemsContainer: "_1sBeAVDWEzemSRJ4BpeGz_",
        GameItemDetails: "_-7GQAa_4IZU5H1UhFegbd",
        ItemBorder: "_1oYLCpb-LNvI_vfS-6D1MA",
        HeaderContainer: "_3-ySSqT7DC8llzFrlXcjRz",
        HeaderTierColor: "_1E9391cOqD2i6l64Jpqedy",
        Tier1: "_2VQtpIDpQ6w22bLG8KOCML",
        Tier2: "_1fvGnZMpBSF0tzLkLPpL2n",
        Tier3: "_3yu5djqYq121Peizf56DTu",
        Tier4: "_6IOmIcmvqVOC9LMZzyou-",
        Tier5: "_2ucHUdtRQHc4ZilVb8WKlQ",
        Header: "gwV88LcsNcMq-50WVpqZn",
        ItemImage: "_2c4tduzkmwTMln9ENFgXsc",
        HeaderText: "_3IgM3enPUnz0XtlvMlXQR1",
        ItemName: "_2PkkTcXv-WqjUfolJPnm_g",
        GoldPrice: "_2TWG_It4lNuxfU9MB3rIWn",
        GoldIcon: "_3-UKazJJJaej3EFwnKii1b",
        NeutralItemTier: "_3ZeLHRdOHkKNhciYs6GJpe",
        Body: "_3SiZOY8un83UvZkFdhsYdr",
        Stats: "_1a8MBoSjOWST1fwQ4R98mS",
        Stat: "_35-AS6HiZTJdcHddL_IbTc",
        SingleValue: "mvrMNClDTNxbSjcVRCUzd",
        DescriptionContainer: "_2-O-I13jxKWQTLmqffVEqr",
        Description: "_1kQS6njHTUQ5RRZo5zSx8Q",
        DescriptionHeader: "_2WnOKi4-2edWniyV0z2J0e",
        CooldownContainer: "_3vpIOhJiU1WJScPUXnn2qe",
        CooldownIcon: "CTnv-zs-2ou3WBPu8mmAK",
        CooldownText: "_1a0WWINOcn7TM6lcHXXPOM",
        ManaContainer: "_3xv9yCnsByFG3fjPZea_FG",
        ManaIcon: "_1ePZXD3SHUC5Im-P2_zCLj",
        ManaText: "_1d40A1VHm-i7Jgw3aw6g9M",
        HealthContainer: "_3y4oGpTo5XmhjwTQgtB_Ms",
        HealthIcon: "_35E4iBFv86GYwhRsTpRBHA",
        HealthText: "_1w0dMSkQizStodSUbjNIi7",
        Lore: "_3wwIAK7bIHIGP0VGnT9uh3",
        Recipe: "_3OWPkSl87hPqWKDy96uAvn",
        RecipeLabel: "_28hKfoPtMuRK0uuZC0n0wB",
        RecipeImagesContainer: "rGUIWPPW8w1lgL3zTscAk",
        RecipeComponentImage: "pu2EH00leKUEdzU1Qa6fd",
        RecipeCost: "_30Gzfpc4onpjqpJZg7vGfT",
        AudioQualityContainer: "gX9woh7uYwkrtJA-bORMZ",
        AudioQualityImage: "_1Pif1aA5TktfrgFwUacEYI",
        QualityOfLifeSection: "_9rBN7xJvQn3AkW4CWeWsM",
        SectionBackground: "_1Gedjp4z_Qx18HVp1taiti",
        BorderlessImage: "_12CdB6s1N_sEsU6iHH_wKL",
        SettingsHeroImage: "_18voGWpb4PWWIRvpDWkQjL",
        ADHeroImage: "_1tGqDaNi2hMcNFwPWNcEmo",
        PingsHeroImage: "Ojen68OqCjvKVqoVpUvR8",
        FooterImage: "lnzWwT78zc84bOCPRdKwV",
        MinorFeatureListContainer: "_2Zh_sBSNT3qp9RpzfXGrmw",
        MinorFeatureListColumn: "_2hEGWMa11ag2fTZVjMMxlk",
        MinorFeature: "tanqT8Py9IwVqY5V0Kl4_",
        BugfixListContainer: "_1_wDzcX6UH8QxoELnmr1Dx",
        BugFixCategoryTitle: "_3FRVv_DRRo-evEvdL6zxHS",
        BugfixListColumn: "_2qYCIUbQrdDJUkzbk8CfRS",
        BugFix: "_39JQIzmAxW8Es3wrCHWHZl",
        GameplayHighlightsSection: "_2qi4eYmQtyNtgZiPVoXb7",
        ItemFXBlock: "_1tjRCp4KPvOr6KKjQ3Tgnc",
        HeroHighlightsContainer: "_1jUHEXdS3MVXuoFkIY3gKV",
        ComparisonContainer: "WiwosNK5llBmLWxGBcecc",
        MinorImprovementsSection: "_22x4nz-BATSQjXm-lU_WT-",
        TreasurePreviewSection: "AATdy0wweXe0_oGKTkL0E",
      };
    },
    95224: (e, a, t) => {
      "use strict";
      t.d(a, { U: () => m, v: () => o });
      var n = t(7552),
        i = t(85655),
        r = t.n(i),
        l = t(21384),
        s = t(89506),
        c = t(27866);
      const o = ({ image: e, is_new: a }) =>
          n.createElement(
            "div",
            { className: r().ComparisonImage },
            n.createElement(
              "div",
              { className: (0, l.A)(r().ImageLabel, a && r().IsNew) },
              (0, c.Wn)(a ? "#729_new_image" : "#729_old_image"),
            ),
            n.createElement("img", { src: `${s.r.IMG_URL}${e}` }),
          ),
        m = (e) => {
          const [a, t] = (0, n.useState)(0);
          return n.createElement(
            "div",
            { className: r().TabbedMapComparison },
            n.createElement(
              "div",
              { className: r().TabHeader },
              e.labels.map((e, i) =>
                n.createElement(
                  "div",
                  {
                    key: "tab_" + i,
                    className: (0, l.A)(r().Tab, a == i && r().Active),
                    onClick: () => t(i),
                  },
                  (0, c.Wn)(e),
                ),
              ),
            ),
            n.createElement(
              "div",
              { className: r().TabContents },
              n.Children.map(e.children, (e, t) =>
                n.createElement(
                  "div",
                  {
                    key: "tabelement_" + t,
                    className: (0, l.A)(
                      r().TabContentContainer,
                      t == a && r().Active,
                    ),
                  },
                  e,
                ),
              ),
            ),
          );
        };
    },
    88644: (e, a, t) => {
      "use strict";
      t.r(a),
        t.d(a, {
          DownloadIcon: () => I,
          ImageVideoCapsule: () => C,
          InnateIconSmall: () => A,
          PlayIcon: () => B,
          default: () => P,
        });
      var n = t(85608),
        i = t(89506),
        r = t(33706),
        l = t(27866),
        s = t(68446),
        c = t(7552),
        o = t(73202),
        m = t(35927),
        g = t(21384),
        _ = t(76489),
        d = t(47202),
        p = t(36305),
        u = t(26942),
        E = t.n(u),
        b = t(61840),
        N = t(32389),
        h = t(54159),
        T = t(82101),
        y = t(85286),
        v = t.n(y),
        w = t(95224),
        x = t(4665),
        f = t(13837),
        L = t.n(f);
      const S = (0, s.PA)((e) => {
          const a = e.patchnotes?.heroes
              ?.slice()
              .find((a) => a.hero_id == e.nHeroID),
            t = r.B5.Get().getAbilityList(),
            n = r.B5.Get().getHeroData(e.nHeroID);
          if (!n || !a || !t) return null;
          const s = n.name.replace("npc_dota_hero_", ""),
            o =
              a.subsections?.filter((e) => e.style.startsWith("hero_facet")) ||
              [],
            m = o.findIndex((a) => a.title == e.strFacetName);
          if (-1 == m) return !1;
          const _ = 20,
            d = o[m];
          return c.createElement(
            "div",
            { className: (0, g.A)(L().HeroHighlight) },
            c.createElement("div", {
              className: (0, g.A)(L().HeroHighlightBorder),
            }),
            c.createElement(
              "div",
              { className: (0, g.A)(L().HeroHighlightVideoContainer) },
              c.createElement("div", {
                className: (0, g.A)(L().HeroHighlightFloorShadow),
              }),
              c.createElement(C, {
                image: `heroes/renders/${s}.png`,
                posterInVideo: !0,
                video: `heroes/renders/${s}.webm`,
                additionalClassName: (0, g.A)(L().HeroHighlightVideo),
                transform: `scale( ${e.strScale || "1"} ) translateX( ${e.strTranslateX || "0"} ) translateY( ${e.strTranslateY || "0"}`,
              }),
            ),
            c.createElement(
              "div",
              { className: L().FacetsContainer },
              c.createElement(
                "div",
                { className: (0, g.A)(L().HeroNameInfoContainer) },
                c.createElement(
                  "p",
                  {
                    className: (0, g.A)(
                      L().TitleFont,
                      L().TitleMedium,
                      L().HeroName,
                    ),
                  },
                  n.name_loc,
                ),
                c.createElement(
                  "p",
                  {
                    className: (0, g.A)(
                      L().LabelFont,
                      L().LabelSmall,
                      L().HighlightTitle,
                    ),
                  },
                  (0, l.Wn)("#patchnotes_new_facet_new"),
                ),
              ),
              c.createElement(
                "div",
                {
                  key: `hero_${e.nHeroID}_facet_${m}`,
                  className: (0, g.A)(
                    L().Facet,
                    L()[`FacetColor${d.facet_color}`],
                    d.style.includes("NewFacet") && L().NewFacet,
                    d.style.includes("ReworkedFacet") && L().ReworkedFacet,
                  ),
                },
                c.createElement("div", { className: L().NewBorder }),
                c.createElement("div", { className: L().ReworkedBorder }),
                c.createElement(
                  "div",
                  { className: L().ReworkedBadge },
                  (0, l.Wn)("#patchnotes_reworked_facet"),
                ),
                c.createElement("div", { className: L().Background }),
                c.createElement(
                  "div",
                  { className: L().FacetHeader },
                  c.createElement("div", {
                    className: L().BackgroundTexture,
                    style: {
                      backgroundImage: `url( ${i.r.IMG_URL}icons/facets/ripple_texture.png )`,
                    },
                  }),
                  c.createElement(
                    "div",
                    { className: L().HeaderContents },
                    c.createElement(
                      "div",
                      { className: L().IconContainer },
                      c.createElement("div", { className: L().IconBackground }),
                      c.createElement("div", { className: L().IconWash }),
                      c.createElement("img", {
                        className: L().Icon,
                        src: `${i.r.IMG_URL}icons/facets/${d.facet_icon}.png`,
                      }),
                    ),
                    c.createElement("div", { className: L().Name }, d.title),
                  ),
                ),
                d.general_notes?.length > 0 &&
                  c.createElement(
                    "div",
                    { className: L().FacetNotes },
                    d.general_notes.map((a, t) =>
                      c.createElement(
                        "div",
                        {
                          key: `hero_${e.nHeroID}_${t}`,
                          className: L().FacetNote,
                        },
                        c.createElement("div", {
                          className: L().Indent,
                          style: {
                            width: _ * (a.indent_level - 1),
                            minWidth: _ * (a.indent_level - 1),
                          },
                        }),
                        c.createElement("div", {
                          className: (0, g.A)(
                            L().Dot,
                            a.hide_dot && L().IsHidden,
                          ),
                        }),
                        (0, l.Wn)(a.note),
                      ),
                    ),
                  ),
                d.abilities?.length > 0 &&
                  c.createElement(
                    "div",
                    { className: L().FacetAbilities },
                    d.abilities?.map((a, n) => {
                      const r = a.ability_id,
                        s = t.itemabilities.find((e) => e.id == r),
                        o = s
                          ? `abilities/${T.b9.has(d.facet) ? T.b9.get(d.facet) : s.name}`
                          : "icons/innate_icon";
                      return c.createElement(
                        "div",
                        {
                          key: `hero_${e.nHeroID}_facet_${m}_ability_${n}`,
                          className: L().FacetAbility,
                        },
                        c.createElement("img", {
                          className: L().AbilityIcon,
                          onError: (e) =>
                            (e.target.src = `${i.r.IMG_URL}icons/innate_icon.png`),
                          src: `${i.r.IMG_URL}${o}.png`,
                        }),
                        c.createElement(
                          "div",
                          { className: L().RightSide },
                          s &&
                            c.createElement(
                              "div",
                              { className: L().AbilityName },
                              s.name_loc,
                            ),
                          a.ability_notes.map((a, t) =>
                            c.createElement(
                              "div",
                              {
                                key: `hero_${e.nHeroID}_${t}`,
                                className: L().AbilityNote,
                              },
                              c.createElement("div", {
                                className: L().Indent,
                                style: {
                                  width: _ * (a.indent_level - 1),
                                  minWidth: _ * (a.indent_level - 1),
                                },
                              }),
                              c.createElement("div", {
                                className: (0, g.A)(
                                  L().Dot,
                                  a.hide_dot && L().IsHidden,
                                ),
                              }),
                              (0, l.Wn)(a.note),
                            ),
                          ),
                        ),
                      );
                    }),
                  ),
                d.talent_notes?.length > 0 &&
                  c.createElement(
                    "div",
                    { className: L().FacetTalentNotes },
                    c.createElement("div", {
                      className: L().TalentImage,
                      style: {
                        backgroundImage: `url( ${i.r.IMG_URL}icons/talents.svg )`,
                      },
                    }),
                    c.createElement(
                      "div",
                      { className: L().Notes },
                      d.talent_notes.map((e) =>
                        c.createElement(
                          "div",
                          { key: e.note, className: L().NoteElement },
                          c.createElement("div", {
                            className: L().Indent,
                            style: {
                              width: _ * (e.indent_level - 1),
                              minWidth: _ * (e.indent_level - 1),
                            },
                          }),
                          c.createElement("div", {
                            className: (0, g.A)(
                              L().Dot,
                              e.hide_dot && L().IsHidden,
                            ),
                          }),
                          c.createElement(
                            "div",
                            { className: L().Note },
                            (0, l.Wn)(e.note),
                          ),
                          e.info && c.createElement(b.O1, { infoText: e.info }),
                        ),
                      ),
                    ),
                  ),
                c.createElement("div", { className: L().FacetBodyFiller }),
              ),
            ),
          );
        }),
        B = () =>
          c.createElement("div", {
            className: E().ControlIcon,
            style: { backgroundImage: `url( ${i.r.IMG_URL}/icons/play.svg )` },
          }),
        I = () =>
          c.createElement("div", {
            className: E().ControlIcon,
            style: {
              backgroundImage: `url( ${i.r.IMG_URL}/icons/download.svg )`,
            },
          }),
        A = () =>
          c.createElement("div", {
            className: (0, g.A)(E().InnateIconSmall, E().ControlIcon),
            style: {
              backgroundImage: `url( ${i.r.IMG_URL}/icons/innate_icon_small.svg )`,
            },
          });
      const C = (e) => {
        const a = (0, c.useRef)();
        return e.video
          ? c.createElement(
              "video",
              {
                className: (0, g.A)(e.additionalClassName),
                ref: a,
                muted: !0,
                autoPlay: !0,
                preload: "auto",
                loop: !0,
                playsInline: !0,
                poster: `${e.posterInVideo ? i.r.VIDEO_URL : i.r.IMG_URL}${e.image}`,
                style: { transform: e.transform },
              },
              c.createElement("source", {
                type: "video/webm",
                src: `${i.r.VIDEO_URL}${e.video}`,
              }),
            )
          : c.createElement("img", {
              className: (0, g.A)(e.additionalClassName),
              src: `${i.r.IMG_URL}/` + e.image,
            });
      };
      function k(e) {
        let a = "",
          t = !0;
        for (let n = 0; n < e.length; ++n)
          "_" != e[n]
            ? t
              ? ((a += e[n].toUpperCase()), (t = !1))
              : (a += e[n])
            : (t = !0);
        return a;
      }
      const F = (e) => {
          if (!e.special.heading_loc) return null;
          let a = e.special.values_float.map((e, a) =>
              c.createElement(
                "span",
                { key: a, className: E().SingleValue },
                (0, h.F)(e),
              ),
            ),
            t = !1,
            n = null;
          return (
            "+" == e.special.heading_loc[0]
              ? ((n = e.special.heading_loc.slice(1)), (t = !0))
              : (n = e.special.heading_loc),
            "$" == n[0] && (n = "#dota_ability_variable_" + n.slice(1)),
            t
              ? c.createElement(
                  "div",
                  { className: E().Stat },
                  "+ ",
                  a,
                  " ",
                  (0, l.Wn)(n),
                )
              : c.createElement(
                  "div",
                  { className: E().Stat },
                  (0, l.Wn)(n),
                  " ",
                  a,
                )
          );
        },
        W = (0, s.PA)(({ name: e, components: a, recipeCost: t }) => {
          const n = r.B5.Get().getItemList(),
            s = n?.itemabilities.find((a) => a.name == e),
            o = r.B5.Get().getItemData(s?.id);
          if (!o) return null;
          let m = o.desc_loc;
          o.special_values.forEach((e) => {
            let a =
              e.values_float.length > 0 ? (0, h.F)(e.values_float[0]) : "0";
            (m = m.replace("%" + e.name + "%", a)),
              (m = m.replace("%" + e.name.toLowerCase() + "%", a));
          }),
            (m = m.replace(/\%\%/g, "%"));
          let _ = o.special_values?.map((e, a) =>
              c.createElement(F, { key: a, special: e }),
            ),
            d = o.name.replace("item_", ""),
            p = o.item_cost,
            u =
              o.item_neutral_tier >= 0 && o.item_neutral_tier < 5
                ? o.item_neutral_tier + 1
                : -1,
            b = E()["Tier" + u],
            N = o.cooldowns.reduce((e, a) => e + a) > 0,
            T = o.mana_costs.reduce((e, a) => e + a) > 0,
            y =
              !!(o.health_costs && o.health_costs.length > 0) &&
              o.health_costs.reduce((e, a) => e + a) > 0,
            v = a
              ? a.map((e, a) =>
                  c.createElement("img", {
                    key: a,
                    className: E().RecipeComponentImage,
                    src: `${i.r.IMG_URL}/items/${e}.png`,
                  }),
                )
              : [];
          return c.createElement(
            "div",
            { className: E().GameItemDetails },
            c.createElement("div", { className: E().ItemBorder }),
            c.createElement(
              "div",
              { className: (0, g.A)(E().HeaderContainer) },
              u > 0 &&
                c.createElement("div", {
                  className: (0, g.A)(E().HeaderTierColor, b),
                }),
              c.createElement(
                "div",
                { className: (0, g.A)(E().Header) },
                c.createElement("img", {
                  className: E().ItemImage,
                  src: `${i.r.IMG_URL}/items/${d}.png`,
                }),
                c.createElement(
                  "div",
                  { className: E().HeaderText },
                  c.createElement(
                    "div",
                    {
                      className: (0, g.A)(
                        E().ItemName,
                        E().TitleFont,
                        E().TitleExtraSmall,
                      ),
                    },
                    o.name_loc,
                  ),
                  p > 0 &&
                    c.createElement(
                      "div",
                      {
                        className: (0, g.A)(
                          E().GoldPrice,
                          E().LabelFont,
                          E().LabelMedium,
                        ),
                      },
                      c.createElement("img", {
                        className: E().GoldIcon,
                        src: `${i.r.IMG_URL}/icons/gold.png`,
                      }),
                      p,
                    ),
                  u > 0 &&
                    c.createElement(
                      "div",
                      { className: (0, g.A)(E().NeutralItemTier, b) },
                      (0, l.Wn)("#neutral_item_tier", u),
                    ),
                ),
              ),
            ),
            c.createElement(
              "div",
              { className: E().Body },
              c.createElement("div", { className: E().Stats }, _),
              m &&
                c.createElement(
                  "div",
                  { className: E().DescriptionContainer },
                  c.createElement("div", {
                    className: E().Description,
                    dangerouslySetInnerHTML: { __html: m },
                  }),
                  (N || T || y) &&
                    c.createElement(
                      "div",
                      { className: (0, g.A)(E().DescriptionHeader) },
                      T &&
                        c.createElement(
                          "div",
                          { className: E().ManaContainer },
                          c.createElement("div", { className: E().ManaIcon }),
                          c.createElement(
                            "div",
                            { className: E().ManaText },
                            o.mana_costs.map(
                              (e, a) => (a > 0 ? " / " : "") + (0, h.F)(e),
                            ),
                          ),
                        ),
                      y &&
                        c.createElement(
                          "div",
                          { className: E().HealthContainer },
                          c.createElement("div", { className: E().HealthIcon }),
                          c.createElement(
                            "div",
                            { className: E().HealthText },
                            o.health_costs.map(
                              (e, a) => (a > 0 ? " / " : "") + (0, h.F)(e),
                            ),
                          ),
                        ),
                      N &&
                        c.createElement(
                          "div",
                          { className: E().CooldownContainer },
                          c.createElement("div", {
                            className: E().CooldownIcon,
                            style: {
                              backgroundImage: `url( ${i.r.IMG_URL}icons/cooldown.png )`,
                            },
                          }),
                          c.createElement(
                            "div",
                            { className: E().CooldownText },
                            o.cooldowns.map(
                              (e, a) => (a > 0 ? " / " : "") + (0, h.F)(e),
                            ),
                          ),
                        ),
                    ),
                ),
            ),
            v.length > 0 &&
              c.createElement(
                "div",
                { className: E().Recipe },
                c.createElement(
                  "p",
                  {
                    className: (0, g.A)(
                      E().RecipeLabel,
                      E().LabelFont,
                      E().LabelSmall,
                      E().LightGrayText,
                    ),
                  },
                  " ",
                  (0, l.Wn)("#templatepage_recipe"),
                  " ",
                ),
                c.createElement(
                  "div",
                  { className: E().RecipeImagesContainer },
                  v,
                  t &&
                    t > 0 &&
                    c.createElement(
                      "div",
                      { className: E().RecipeCost },
                      " + ",
                      t,
                      " ",
                    ),
                  t &&
                    t > 0 &&
                    c.createElement("img", {
                      className: E().RecipeComponentImage,
                      src: `${i.r.IMG_URL}/items/recipe.png`,
                    }),
                ),
              ),
          );
        }),
        H =
          ((0, s.PA)(({ patchnotes: e, heroname: a }) => {
            const t = r.B5.Get().getHeroList(),
              n = t?.heroes.find(
                (e) => e.name.replace("npc_dota_hero_", "") == a,
              );
            return n
              ? c.createElement(
                  "div",
                  { className: (0, g.A)(E().HeroRework, E()[k(a)]) },
                  c.createElement(
                    "div",
                    {
                      className: (0, g.A)(
                        E().HeroName,
                        E().TitleFont,
                        E().TitleSmall,
                      ),
                    },
                    (0, l.Wn)(n.name_loc),
                  ),
                  c.createElement(
                    "div",
                    {
                      className: (0, g.A)(
                        E().ReworkDescription,
                        E().DisplayFont,
                        E().DisplayExtraSmall,
                        E().LightGrayText,
                      ),
                    },
                    (0, l.Wn)("#new_frontiers_major_gameplay_hero_rework_" + a),
                  ),
                  c.createElement(
                    "div",
                    { className: E().HeroImageContainer },
                    c.createElement("div", { className: E().HeroShadow }),
                    c.createElement(T.sG, {
                      heroname: a,
                      portraitClassName: E().HeroReworkPortrait,
                      videoClassName: E().HeroReworkPortraitVideo,
                    }),
                  ),
                  c.createElement(
                    "div",
                    { className: E().HeroReworkPatchNotes },
                    c.createElement(b.fX, {
                      patchnotes: e,
                      heroname: a,
                      heroClassName: E().HeroReworkPatchNotesInner,
                    }),
                  ),
                )
              : null;
          }),
          (e) =>
            c.createElement(
              "div",
              { className: (0, g.A)(E().BugFix) },
              c.createElement(
                "p",
                { className: (0, g.A)(E().BodyFont, E().BodyLarge) },
                (0, l.Wn)(e.description),
              ),
            )),
        D = ({
          index: e,
          video: a,
          name: n,
          heroname: r,
          autoplay: l,
          onSlideIn: s,
        }) => {
          const o = (0, c.useContext)(N.Yc),
            m = (0, c.useRef)();
          return (
            (0, c.useEffect)(() => {
              function a() {
                m && m.current && o.state.currentSlide == e
                  ? m.current.play()
                  : m && m.current && m.current.pause(),
                  o.state.currentSlide == e && s(n, r);
              }
              return o.subscribe(a), () => o.unsubscribe(a);
            }, [o, e, n, r, s]),
            c.createElement(
              "div",
              { className: E().SlideContainer },
              (function () {
                const e = t(99769),
                  a =
                    -1 != navigator.userAgent.toLowerCase().indexOf("safari") &&
                    -1 !=
                      navigator.userAgent.toLowerCase().indexOf("macintosh");
                return e || a;
              })()
                ? c.createElement("img", {
                    className: E().TreasureVideo,
                    src: `${i.r.VIDEO_URL}/springcleaning2025/treasure/${a}.png`,
                  })
                : c.createElement(
                    "video",
                    {
                      ref: m,
                      className: E().TreasureVideo,
                      muted: !0,
                      autoPlay: l,
                      preload: "auto",
                      loop: !0,
                      playsInline: !0,
                      poster: `${i.r.VIDEO_URL}/springcleaning2025/treasure/${a}.png`,
                    },
                    c.createElement("source", {
                      type: "video/webm",
                      src: `${i.r.VIDEO_URL}/springcleaning2025/treasure/${a}.webm`,
                    }),
                    c.createElement("source", {
                      type: 'video/mp4; codecs="hvc1"',
                      src: `${i.r.VIDEO_URL}/springcleaning2025/treasure/${a}.mov`,
                    }),
                  ),
            )
          );
        },
        G = (e) =>
          c.createElement(
            "div",
            {
              className: e
                ? (0, g.A)(E().SectionDivider, e)
                : (0, g.A)(E().SectionDivider),
            },
            c.createElement("div", { className: E().Pattern }),
            c.createElement("div", { className: E().Overlay }),
            c.createElement("div", { className: E().TopDash }),
            c.createElement("div", { className: E().BottomDash }),
          ),
        M = () =>
          c.createElement(
            "div",
            { className: E().SubsectionDivider },
            c.createElement("div", { className: E().TopDash }),
            c.createElement("div", { className: E().Background }),
          );
      let R = class extends c.Component {
        parallaxContainerRef = c.createRef();
        videoRef = c.createRef();
        constructor(e) {
          super(e),
            (this.state = {
              treasureName: "#springcleaning2025_treasure_hero_pudge_set",
              heroName: "#springcleaning2025_treasure_hero_pudge",
              bPlayingVideo: !1,
            });
        }
        setPlayingVideo(e) {
          this.setState({ bPlayingVideo: e }),
            e ? this.videoRef.current.play() : this.videoRef.current.pause();
        }
        handleScroll = (e) => {
          v().refresh();
        };
        componentDidMount() {
          this.parallaxContainerRef.current.addEventListener(
            "scroll",
            this.handleScroll,
          ),
            this.handleScroll(void 0);
        }
        componentWillUnmount() {
          this.parallaxContainerRef.current.removeEventListener(
            "scroll",
            this.handleScroll,
          );
        }
        convertAbilityDesc(e) {
          if (!e) return null;
          let a = e.desc_loc;
          return (
            e.special_values.forEach((e) => {
              let t =
                e.values_float.length > 0 ? (0, h.F)(e.values_float[0]) : "0";
              (a = a.replace("%" + e.name + "%", t)),
                (a = a.replace("%" + e.name.toLowerCase() + "%", t));
            }),
            (a = a.replace(/\%\%/g, "%")),
            (a = a.replace(/<h2>/g, "<b>")),
            (a = a.replace(/<\/h2>/g, "</b>")),
            (a = a.replace(/<h1>/g, "<b>")),
            (a = a.replace(/<\/h1>/g, "</b>\n\n")),
            (0, l.Wn)(a)
          );
        }
        render() {
          const e = _.o.getPatchNotes("7.39", i.r.LANGUAGE);
          let a = (0, m.ww)((0, m.sf)(i.r.LANGUAGE));
          return (
            "zh-cn" === a ? (a = "zh-Hans") : "zh-tw" === a && (a = "zh-Hant"),
            c.createElement(
              "div",
              { id: "Springcleaning2025", className: E().Springcleaning2025 },
              c.createElement(
                o.mg,
                null,
                c.createElement(
                  "title",
                  null,
                  (0, l.Wn)("#springcleaning2025_website_title"),
                ),
              ),
              c.createElement(
                "div",
                {
                  ref: this.parallaxContainerRef,
                  className: (0, g.A)(E().PageContainer, E().Parallax),
                },
                c.createElement(d.A, { bOverlapping: !0 }),
                c.createElement(
                  "div",
                  { className: (0, g.A)(E().HeaderSection) },
                  c.createElement("div", {
                    className: (0, g.A)(E().BackgroundGradient),
                  }),
                  c.createElement(
                    "div",
                    { className: E().WebsiteSectionInner },
                    c.createElement(C, {
                      image: "springcleaning2025/header/header_hero.png",
                      additionalClassName: (0, g.A)(E().HeaderHeroImage),
                    }),
                    c.createElement(
                      "div",
                      { className: E().HeaderTextSection },
                      c.createElement(
                        "p",
                        {
                          className: (0, g.A)(
                            E().WebsiteTitle,
                            E().LabelFont,
                            E().LabelLarge,
                          ),
                        },
                        (0, l.Wn)("#springcleaning2025_website_year"),
                      ),
                      c.createElement(
                        "p",
                        {
                          className: (0, g.A)(
                            E().WebsiteTitle,
                            E().TitleFont,
                            E().TitleExtraLarge,
                          ),
                        },
                        (0, l.Wn)("#springcleaning2025_website_title"),
                      ),
                      c.createElement(
                        "p",
                        {
                          className: (0, g.A)(
                            E().WebsiteIntro,
                            E().DisplayFont,
                            E().DisplayMedium,
                          ),
                        },
                        (0, l.Wn)("#springcleaning2025_website_introduction"),
                      ),
                    ),
                    c.createElement(
                      "div",
                      { className: E().Grid_3 },
                      c.createElement(
                        "div",
                        { className: E().TextImageBlockVertical },
                        c.createElement(C, {
                          image: "springcleaning2025/header/gameplay.png",
                          additionalClassName: (0, g.A)(E().HeaderFeatureImage),
                        }),
                        c.createElement(
                          "div",
                          { className: E().TextBlock },
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockTitle,
                                E().TitleFont,
                                E().TitleSmall,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_abovethefold_patch_header",
                            ),
                          ),
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockDescription,
                                E().BodyFont,
                                E().BodyLarge,
                                E().LightGrayText,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_abovethefold_patch_body",
                            ),
                          ),
                        ),
                      ),
                      c.createElement(
                        "div",
                        { className: E().TextImageBlockVertical },
                        c.createElement(C, {
                          image: "springcleaning2025/header/treasure.png",
                          additionalClassName: (0, g.A)(E().HeaderFeatureImage),
                        }),
                        c.createElement(
                          "div",
                          { className: E().TextBlock },
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockTitle,
                                E().TitleFont,
                                E().TitleSmall,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_abovethefold_treasure_header",
                            ),
                          ),
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockDescription,
                                E().BodyFont,
                                E().BodyLarge,
                                E().LightGrayText,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_abovethefold_treasure_body",
                            ),
                          ),
                        ),
                      ),
                      c.createElement(
                        "div",
                        { className: E().TextImageBlockVertical },
                        c.createElement(C, {
                          image:
                            "springcleaning2025/header/quality_of_life.png",
                          additionalClassName: (0, g.A)(E().HeaderFeatureImage),
                        }),
                        c.createElement(
                          "div",
                          { className: E().TextBlock },
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockTitle,
                                E().TitleFont,
                                E().TitleSmall,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_abovethefold_features_header",
                            ),
                          ),
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockDescription,
                                E().BodyFont,
                                E().BodyLarge,
                                E().LightGrayText,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_abovethefold_features_body",
                            ),
                          ),
                        ),
                      ),
                    ),
                  ),
                ),
                G(),
                " ",
                c.createElement(
                  "div",
                  {
                    id: "QualityOfLifeSection",
                    className: (0, g.A)(
                      E().WebsiteSection,
                      E().QualityOfLifeSection,
                    ),
                  },
                  c.createElement(C, {
                    image: "springcleaning2025/backgrounds/qol_background.png",
                    additionalClassName: (0, g.A)(E().SectionBackground),
                  }),
                  c.createElement("div", { className: E().TopShadow }),
                  c.createElement(
                    "div",
                    { className: E().WebsiteSectionInner },
                    c.createElement(
                      "div",
                      { className: (0, g.A)(E().WebsiteSectionHeader) },
                      c.createElement(
                        "h2",
                        {
                          className: (0, g.A)(
                            E().SectionHeaderLabel,
                            E().TitleFont,
                            E().TitleExtraLarge,
                          ),
                        },
                        (0, l.Wn)(
                          "#springcleaning2025_website_section_header_features",
                        ),
                      ),
                      c.createElement(
                        "p",
                        {
                          className: (0, g.A)(
                            E().DisplayFont,
                            E().DisplayMedium,
                          ),
                        },
                        (0, l.Wn)(
                          "#springcleaning2025_website_section_header_introduction",
                        ),
                      ),
                    ),
                    M(),
                    " ",
                    c.createElement(
                      "div",
                      {
                        className: (0, g.A)(
                          E().WebsiteSectionHeader,
                          E().FeatureHeader,
                        ),
                      },
                      c.createElement(
                        "h2",
                        {
                          className: (0, g.A)(
                            E().SectionHeaderLabel,
                            E().TitleFont,
                            E().TitleLarge,
                          ),
                        },
                        (0, l.Wn)(
                          "#springcleaning2025_website_features_group_focus_on_hero",
                        ),
                      ),
                      c.createElement(
                        "p",
                        {
                          className: (0, g.A)(
                            E().SectionDescriptionLabel,
                            E().DisplayFont,
                            E().DisplaySmall,
                            E().LightGrayText,
                          ),
                        },
                        (0, l.Wn)(
                          "#springcleaning2025_website_features_group_focus_on_hero_summary",
                        ),
                      ),
                    ),
                    c.createElement(
                      "div",
                      { className: E().Grid_2 },
                      c.createElement(
                        "div",
                        { className: E().TextImageBlockVertical },
                        c.createElement(C, {
                          image: "springcleaning2025/shop/shop_team_items.jpg",
                          additionalClassName: (0, g.A)(E().ImageShadowMedium),
                        }),
                        c.createElement(
                          "div",
                          { className: (0, g.A)(E().TextSection) },
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockTitle,
                                E().LabelFont,
                                E().LabelExtraLarge,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_team_items",
                            ),
                          ),
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockDescription,
                                E().BodyFont,
                                E().BodyLarge,
                                E().LightGrayText,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_team_items_desc",
                            ),
                          ),
                        ),
                      ),
                      c.createElement(
                        "div",
                        { className: E().TextImageBlockVertical },
                        c.createElement(C, {
                          image:
                            "springcleaning2025/courier/courier_auto_deliver.png",
                          additionalClassName: (0, g.A)(E().BorderlessImage),
                        }),
                        c.createElement(
                          "div",
                          { className: (0, g.A)(E().TextSection) },
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockTitle,
                                E().LabelFont,
                                E().LabelExtraLarge,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_auto_deliver",
                            ),
                          ),
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockDescription,
                                E().BodyFont,
                                E().BodyLarge,
                                E().LightGrayText,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_auto_deliver_desc",
                            ),
                          ),
                        ),
                      ),
                    ),
                    c.createElement(
                      "div",
                      { className: E().Grid_2 },
                      c.createElement(
                        "div",
                        { className: E().TextImageBlockVertical },
                        c.createElement(C, {
                          image:
                            "springcleaning2025/shop/courier_mark_for_buy.jpg",
                          additionalClassName: (0, g.A)(E().ImageShadowMedium),
                        }),
                        c.createElement(
                          "div",
                          { className: (0, g.A)(E().TextSection) },
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockTitle,
                                E().LabelFont,
                                E().LabelExtraLarge,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_quickbuy",
                            ),
                          ),
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockDescription,
                                E().BodyFont,
                                E().BodyLarge,
                                E().LightGrayText,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_quickbuy_desc",
                            ),
                          ),
                        ),
                      ),
                      c.createElement(
                        "div",
                        { className: E().TextImageBlockVertical },
                        c.createElement(C, {
                          image:
                            "springcleaning2025/shop/shop_pregame_items.jpg",
                          additionalClassName: (0, g.A)(E().ImageShadowMedium),
                        }),
                        c.createElement(
                          "div",
                          { className: (0, g.A)(E().TextSection) },
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockTitle,
                                E().LabelFont,
                                E().LabelExtraLarge,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_pregame_shop",
                            ),
                          ),
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockDescription,
                                E().BodyFont,
                                E().BodyLarge,
                                E().LightGrayText,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_pregame_shop_desc",
                            ),
                          ),
                        ),
                      ),
                    ),
                    c.createElement(
                      "div",
                      { className: E().Grid_3 },
                      c.createElement(
                        "div",
                        { className: E().TextImageBlockVertical },
                        c.createElement(C, {
                          image: "springcleaning2025/shop/buyback_tooltip.jpg",
                          additionalClassName: (0, g.A)(E().ImageShadowMedium),
                        }),
                        c.createElement(
                          "div",
                          { className: (0, g.A)(E().TextSection) },
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockTitle,
                                E().LabelFont,
                                E().LabelExtraLarge,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_buyback_tooltip",
                            ),
                          ),
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockDescription,
                                E().BodyFont,
                                E().BodyLarge,
                                E().LightGrayText,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_buyback_tooltip_desc",
                            ),
                          ),
                        ),
                      ),
                      c.createElement(
                        "div",
                        { className: E().TextImageBlockVertical },
                        c.createElement(C, {
                          image: "springcleaning2025/shop/pregame_clarity.jpg",
                          additionalClassName: (0, g.A)(E().ImageShadowMedium),
                        }),
                        c.createElement(
                          "div",
                          { className: (0, g.A)(E().TextSection) },
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockTitle,
                                E().LabelFont,
                                E().LabelExtraLarge,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_identify_yourself",
                            ),
                          ),
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockDescription,
                                E().BodyFont,
                                E().BodyLarge,
                                E().LightGrayText,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_identify_yourself_desc",
                            ),
                          ),
                        ),
                      ),
                      c.createElement(
                        "div",
                        { className: E().TextImageBlockVertical },
                        c.createElement(C, {
                          image:
                            "springcleaning2025/shop/shop_dota_context_menu.jpg",
                          additionalClassName: (0, g.A)(E().ImageShadowMedium),
                        }),
                        c.createElement(
                          "div",
                          { className: (0, g.A)(E().TextSection) },
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockTitle,
                                E().LabelFont,
                                E().LabelExtraLarge,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_item_context_menu",
                            ),
                          ),
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockDescription,
                                E().BodyFont,
                                E().BodyLarge,
                                E().LightGrayText,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_item_context_menu_desc",
                            ),
                          ),
                        ),
                      ),
                    ),
                    c.createElement(
                      "div",
                      {
                        className: (0, g.A)(
                          E().TextImageBlockHorizontal,
                          E().WideImage,
                        ),
                      },
                      c.createElement(C, {
                        image:
                          "springcleaning2025/shop/shop_dota_plus_recommender.jpg",
                        additionalClassName: (0, g.A)(E().ImageShadowMedium),
                      }),
                      c.createElement(
                        "div",
                        { className: E().TextBlock },
                        c.createElement(
                          "p",
                          {
                            className: (0, g.A)(
                              E().BlockTitle,
                              E().LabelFont,
                              E().LabelExtraLarge,
                            ),
                          },
                          (0, l.Wn)(
                            "#springcleaning2025_website_features_entry_dotaplus_shop",
                          ),
                        ),
                        c.createElement(
                          "p",
                          {
                            className: (0, g.A)(
                              E().BlockDescription,
                              E().BodyFont,
                              E().BodyLarge,
                              E().LightGrayText,
                            ),
                          },
                          (0, l.Wn)(
                            "#springcleaning2025_website_features_entry_dotaplus_shop_desc",
                          ),
                        ),
                        c.createElement(
                          "div",
                          { className: E().DotaPlusBadge },
                          c.createElement("img", {
                            src: `${i.r.IMG_URL}/icons/dota_plus.png`,
                          }),
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().LabelFont,
                                E().LabelSmall,
                              ),
                            },
                            (0, l.Wn)("#dota_plus"),
                          ),
                        ),
                      ),
                    ),
                    M(),
                    " ",
                    c.createElement(
                      "div",
                      {
                        className: (0, g.A)(
                          E().WebsiteSectionHeader,
                          E().FeatureHeader,
                        ),
                      },
                      c.createElement(
                        "h2",
                        {
                          className: (0, g.A)(
                            E().SectionHeaderLabel,
                            E().TitleFont,
                            E().TitleLarge,
                          ),
                        },
                        (0, l.Wn)(
                          "#springcleaning2025_website_features_group_comms",
                        ),
                      ),
                      c.createElement(
                        "p",
                        {
                          className: (0, g.A)(
                            E().SectionDescriptionLabel,
                            E().DisplayFont,
                            E().DisplaySmall,
                            E().LightGrayText,
                          ),
                        },
                        (0, l.Wn)(
                          "#springcleaning2025_website_features_group_comms_summary",
                        ),
                      ),
                    ),
                    c.createElement(
                      "div",
                      { className: E().InnerContainer },
                      c.createElement(
                        "div",
                        { className: (0, g.A)(E().Grid_2, E().PingsGrid) },
                        c.createElement(
                          "div",
                          { className: E().TextImageBlockVertical },
                          c.createElement(C, {
                            image:
                              "springcleaning2025/pings/ping_example_standard.jpg",
                            video: "springcleaning2025/pings.webm",
                            additionalClassName: (0, g.A)(
                              E().ImageShadowMedium,
                            ),
                          }),
                        ),
                        c.createElement(
                          "div",
                          { className: E().TextImageBlockVertical },
                          c.createElement(C, {
                            image: "springcleaning2025/pings/ping_wheel.jpg",
                            video: "springcleaning2025/ping_wheel.webm",
                            additionalClassName: (0, g.A)(
                              E().ImageShadowMedium,
                            ),
                          }),
                        ),
                      ),
                      c.createElement(
                        "div",
                        { className: E().TextBlock },
                        c.createElement(
                          "p",
                          {
                            className: (0, g.A)(
                              E().BlockTitle,
                              E().LabelFont,
                              E().LabelExtraLarge,
                            ),
                          },
                          (0, l.Wn)(
                            "#springcleaning2025_website_features_entry_pings",
                          ),
                        ),
                        c.createElement(
                          "p",
                          {
                            className: (0, g.A)(
                              E().BlockDescription,
                              E().BodyFont,
                              E().BodyLarge,
                              E().LightGrayText,
                            ),
                          },
                          (0, l.Wn)(
                            "#springcleaning2025_website_features_entry_pings_desc",
                          ),
                        ),
                      ),
                    ),
                    c.createElement(
                      "div",
                      { className: E().Grid_3 },
                      c.createElement(
                        "div",
                        { className: E().TextImageBlockVertical },
                        c.createElement(C, {
                          image:
                            "springcleaning2025/pings/item_pickup_effects.jpg",
                          additionalClassName: (0, g.A)(E().ImageShadowMedium),
                        }),
                        c.createElement(
                          "div",
                          { className: E().TextBlock },
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockTitle,
                                E().LabelFont,
                                E().LabelExtraLarge,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_pickup_effects",
                            ),
                          ),
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockDescription,
                                E().BodyFont,
                                E().BodyLarge,
                                E().LightGrayText,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_pickup_effects_desc",
                            ),
                          ),
                        ),
                      ),
                      c.createElement(
                        "div",
                        { className: E().TextImageBlockVertical },
                        c.createElement(C, {
                          image:
                            "springcleaning2025/pings/minimap_route_display.jpg",
                          additionalClassName: (0, g.A)(E().ImageShadowMedium),
                        }),
                        c.createElement(
                          "div",
                          { className: E().TextBlock },
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockTitle,
                                E().LabelFont,
                                E().LabelExtraLarge,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_minimap_route",
                            ),
                          ),
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockDescription,
                                E().BodyFont,
                                E().BodyLarge,
                                E().LightGrayText,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_minimap_route_desc",
                            ),
                          ),
                        ),
                      ),
                      c.createElement(
                        "div",
                        { className: E().TextImageBlockVertical },
                        c.createElement(C, {
                          image: "springcleaning2025/pings/status_labels.jpg",
                          additionalClassName: (0, g.A)(E().ImageShadowMedium),
                        }),
                        c.createElement(
                          "div",
                          { className: E().TextBlock },
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockTitle,
                                E().LabelFont,
                                E().LabelExtraLarge,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_status_labels",
                            ),
                          ),
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockDescription,
                                E().BodyFont,
                                E().BodyLarge,
                                E().LightGrayText,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_status_labels_desc",
                            ),
                          ),
                        ),
                      ),
                    ),
                    c.createElement(
                      "div",
                      {
                        className: (0, g.A)(
                          E().InnerContainer,
                          E().AudioQualityContainer,
                        ),
                      },
                      c.createElement(C, {
                        image: "springcleaning2025/pings/audio_quality.png",
                        additionalClassName: (0, g.A)(
                          E().FullWidthImage,
                          E().AudioQualityImage,
                        ),
                      }),
                      c.createElement(
                        "div",
                        { className: E().TextBlock },
                        c.createElement(
                          "p",
                          {
                            className: (0, g.A)(
                              E().BlockTitle,
                              E().LabelFont,
                              E().LabelExtraLarge,
                            ),
                          },
                          (0, l.Wn)(
                            "#springcleaning2025_website_features_entry_voice_chat",
                          ),
                        ),
                        c.createElement(
                          "p",
                          {
                            className: (0, g.A)(
                              E().BlockDescription,
                              E().BodyFont,
                              E().BodyLarge,
                              E().LightGrayText,
                            ),
                          },
                          (0, l.Wn)(
                            "#springcleaning2025_website_features_entry_voice_chat_desc",
                          ),
                        ),
                      ),
                    ),
                    M(),
                    " ",
                    c.createElement(
                      "div",
                      {
                        className: (0, g.A)(
                          E().WebsiteSectionHeader,
                          E().FeatureHeader,
                        ),
                      },
                      c.createElement(
                        "h2",
                        {
                          className: (0, g.A)(
                            E().SectionHeaderLabel,
                            E().TitleFont,
                            E().TitleLarge,
                          ),
                        },
                        (0, l.Wn)(
                          "#springcleaning2025_website_features_group_control",
                        ),
                      ),
                      c.createElement(
                        "p",
                        {
                          className: (0, g.A)(
                            E().SectionDescriptionLabel,
                            E().DisplayFont,
                            E().DisplaySmall,
                            E().LightGrayText,
                          ),
                        },
                        (0, l.Wn)(
                          "#springcleaning2025_website_features_group_control_summary",
                        ),
                      ),
                    ),
                    c.createElement(C, {
                      image: "springcleaning2025/settings/settings_hero.png",
                      additionalClassName: (0, g.A)(
                        E().FullWidthImage,
                        E().SettingsHeroImage,
                      ),
                    }),
                    c.createElement(
                      "div",
                      { className: E().TextBlock },
                      c.createElement(
                        "p",
                        {
                          className: (0, g.A)(
                            E().BlockTitle,
                            E().LabelFont,
                            E().LabelExtraLarge,
                          ),
                        },
                        (0, l.Wn)(
                          "#springcleaning2025_website_features_entry_settings",
                        ),
                      ),
                      c.createElement(
                        "p",
                        {
                          className: (0, g.A)(
                            E().BlockDescription,
                            E().BodyFont,
                            E().BodyLarge,
                            E().LightGrayText,
                          ),
                        },
                        (0, l.Wn)(
                          "#springcleaning2025_website_features_entry_settings_desc",
                        ),
                      ),
                    ),
                    c.createElement(
                      "div",
                      { className: E().Grid_2 },
                      c.createElement(
                        "div",
                        { className: E().TextImageBlockVertical },
                        c.createElement(C, {
                          image:
                            "springcleaning2025/settings/settings_search.jpg",
                          video: "springcleaning2025/settings_search.webm",
                          additionalClassName: (0, g.A)(E().ImageShadowMedium),
                        }),
                        c.createElement(
                          "div",
                          { className: (0, g.A)(E().TextSection) },
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockTitle,
                                E().LabelFont,
                                E().LabelExtraLarge,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_settings_search",
                            ),
                          ),
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockDescription,
                                E().BodyFont,
                                E().BodyLarge,
                                E().LightGrayText,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_settings_search_desc",
                            ),
                          ),
                        ),
                      ),
                      c.createElement(
                        "div",
                        { className: E().TextImageBlockVertical },
                        c.createElement(C, {
                          image: "springcleaning2025/settings/settings_new.jpg",
                          additionalClassName: (0, g.A)(E().ImageShadowMedium),
                        }),
                        c.createElement(
                          "div",
                          { className: (0, g.A)(E().TextSection) },
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockTitle,
                                E().LabelFont,
                                E().LabelExtraLarge,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_new_settings",
                            ),
                          ),
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockDescription,
                                E().BodyFont,
                                E().BodyLarge,
                                E().LightGrayText,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_new_settings_desc",
                            ),
                          ),
                        ),
                      ),
                    ),
                    c.createElement(
                      "div",
                      { className: E().Grid_2 },
                      c.createElement(
                        "div",
                        { className: E().TextImageBlockVertical },
                        c.createElement(C, {
                          image: "springcleaning2025/pings/ping_customize.jpg",
                          additionalClassName: (0, g.A)(E().ImageShadowMedium),
                        }),
                        c.createElement(
                          "div",
                          { className: (0, g.A)(E().TextSection) },
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockTitle,
                                E().LabelFont,
                                E().LabelExtraLarge,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_chatwheel_ui",
                            ),
                          ),
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockDescription,
                                E().BodyFont,
                                E().BodyLarge,
                                E().LightGrayText,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_chatwheel_ui_desc",
                            ),
                          ),
                        ),
                      ),
                      c.createElement(
                        "div",
                        { className: E().TextImageBlockVertical },
                        c.createElement(C, {
                          image:
                            "springcleaning2025/settings/manage_notifications.jpg",
                          additionalClassName: (0, g.A)(E().ImageShadowMedium),
                        }),
                        c.createElement(
                          "div",
                          { className: (0, g.A)(E().TextSection) },
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockTitle,
                                E().LabelFont,
                                E().LabelExtraLarge,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_notifications",
                            ),
                          ),
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockDescription,
                                E().BodyFont,
                                E().BodyLarge,
                                E().LightGrayText,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_notifications_desc",
                            ),
                          ),
                        ),
                      ),
                    ),
                    M(),
                    " ",
                    c.createElement(
                      "div",
                      {
                        className: (0, g.A)(
                          E().WebsiteSectionHeader,
                          E().FeatureHeader,
                        ),
                      },
                      c.createElement(
                        "h2",
                        {
                          className: (0, g.A)(
                            E().SectionHeaderLabel,
                            E().TitleFont,
                            E().TitleLarge,
                          ),
                        },
                        (0, l.Wn)(
                          "#springcleaning2025_website_features_group_ability_draft",
                        ),
                      ),
                      c.createElement(
                        "p",
                        {
                          className: (0, g.A)(
                            E().SectionDescriptionLabel,
                            E().DisplayFont,
                            E().DisplaySmall,
                            E().LightGrayText,
                          ),
                        },
                        (0, l.Wn)(
                          "#springcleaning2025_website_features_group_ability_draft_summary",
                        ),
                      ),
                    ),
                    c.createElement(C, {
                      image:
                        "springcleaning2025/ability_draft/ability_draft_hero.png",
                      additionalClassName: (0, g.A)(
                        E().FullWidthImage,
                        E().ADHeroImage,
                      ),
                    }),
                    c.createElement(
                      "div",
                      { className: E().Grid_3 },
                      c.createElement(
                        "div",
                        { className: E().TextImageBlockVertical },
                        c.createElement(C, {
                          image:
                            "springcleaning2025/ability_draft/ability_draft_hero_drafting.png",
                          additionalClassName: (0, g.A)(E().BorderlessImage),
                        }),
                        c.createElement(
                          "div",
                          { className: (0, g.A)(E().TextSection) },
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockTitle,
                                E().LabelFont,
                                E().LabelExtraLarge,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_heroes",
                            ),
                          ),
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockDescription,
                                E().BodyFont,
                                E().BodyLarge,
                                E().LightGrayText,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_heroes_desc",
                            ),
                          ),
                        ),
                      ),
                      c.createElement(
                        "div",
                        { className: E().TextImageBlockVertical },
                        c.createElement(C, {
                          image:
                            "springcleaning2025/ability_draft/ability_draft_facets.png",
                          additionalClassName: (0, g.A)(E().BorderlessImage),
                        }),
                        c.createElement(
                          "div",
                          { className: (0, g.A)(E().TextSection) },
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockTitle,
                                E().LabelFont,
                                E().LabelExtraLarge,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_facets",
                            ),
                          ),
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockDescription,
                                E().BodyFont,
                                E().BodyLarge,
                                E().LightGrayText,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_facets_desc",
                            ),
                          ),
                        ),
                      ),
                      c.createElement(
                        "div",
                        { className: E().TextImageBlockVertical },
                        c.createElement(C, {
                          image:
                            "springcleaning2025/ability_draft/ability_draft_cleanup.png",
                          additionalClassName: (0, g.A)(E().BorderlessImage),
                        }),
                        c.createElement(
                          "div",
                          { className: (0, g.A)(E().TextSection) },
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockTitle,
                                E().LabelFont,
                                E().LabelExtraLarge,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_cleanup",
                            ),
                          ),
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockDescription,
                                E().BodyFont,
                                E().BodyLarge,
                                E().LightGrayText,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_cleanup_desc",
                            ),
                          ),
                        ),
                      ),
                    ),
                    M(),
                    " ",
                    c.createElement(
                      "div",
                      {
                        className: (0, g.A)(
                          E().WebsiteSectionHeader,
                          E().FeatureHeader,
                        ),
                      },
                      c.createElement(
                        "h2",
                        {
                          className: (0, g.A)(
                            E().SectionHeaderLabel,
                            E().TitleFont,
                            E().TitleLarge,
                          ),
                        },
                        (0, l.Wn)(
                          "#springcleaning2025_website_features_group_everything_else",
                        ),
                      ),
                      c.createElement(
                        "p",
                        {
                          className: (0, g.A)(
                            E().SectionDescriptionLabel,
                            E().DisplayFont,
                            E().DisplaySmall,
                            E().LightGrayText,
                          ),
                        },
                        (0, l.Wn)(
                          "#springcleaning2025_website_features_group_everything_else_summary",
                        ),
                      ),
                    ),
                    c.createElement(
                      "div",
                      { className: E().Grid_2 },
                      c.createElement(
                        "div",
                        { className: E().TextImageBlockVertical },
                        c.createElement(C, {
                          image:
                            "springcleaning2025/performance/performance.png",
                          additionalClassName: (0, g.A)(E().BorderlessImage),
                        }),
                        c.createElement(
                          "div",
                          { className: (0, g.A)(E().TextSection) },
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockTitle,
                                E().LabelFont,
                                E().LabelExtraLarge,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_perf",
                            ),
                          ),
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockDescription,
                                E().BodyFont,
                                E().BodyLarge,
                                E().LightGrayText,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_perf_desc",
                            ),
                          ),
                        ),
                      ),
                      c.createElement(
                        "div",
                        { className: E().TextImageBlockVertical },
                        c.createElement(C, {
                          image:
                            "springcleaning2025/graphs/graphs_player_damage.png",
                          additionalClassName: (0, g.A)(E().BorderlessImage),
                        }),
                        c.createElement(
                          "div",
                          { className: (0, g.A)(E().TextSection) },
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockTitle,
                                E().LabelFont,
                                E().LabelExtraLarge,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_graphs",
                            ),
                          ),
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockDescription,
                                E().BodyFont,
                                E().BodyLarge,
                                E().LightGrayText,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_graphs_desc",
                            ),
                          ),
                        ),
                      ),
                    ),
                    c.createElement(
                      "div",
                      { className: E().Grid_3 },
                      c.createElement(
                        "div",
                        { className: E().TextImageBlockVertical },
                        c.createElement(C, {
                          image:
                            "springcleaning2025/cornucopia/arcana_mvp_backgrounds.jpg",
                          additionalClassName: (0, g.A)(E().ImageShadowMedium),
                        }),
                        c.createElement(
                          "div",
                          { className: E().TextBlock },
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockTitle,
                                E().LabelFont,
                                E().LabelExtraLarge,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_arcana_mvps",
                            ),
                          ),
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockDescription,
                                E().BodyFont,
                                E().BodyLarge,
                                E().LightGrayText,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_arcana_mvps_desc",
                            ),
                          ),
                        ),
                      ),
                      c.createElement(
                        "div",
                        { className: E().TextImageBlockVertical },
                        c.createElement(C, {
                          image:
                            "springcleaning2025/cornucopia/dead_neutral_creeps.jpg",
                          additionalClassName: (0, g.A)(E().ImageShadowMedium),
                        }),
                        c.createElement(
                          "div",
                          { className: E().TextBlock },
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockTitle,
                                E().LabelFont,
                                E().LabelExtraLarge,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_this_bug_is_old_enough_to_drive",
                            ),
                          ),
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockDescription,
                                E().BodyFont,
                                E().BodyLarge,
                                E().LightGrayText,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_this_bug_is_old_enough_to_drive_desc",
                            ),
                          ),
                        ),
                      ),
                      c.createElement(
                        "div",
                        { className: E().TextImageBlockVertical },
                        c.createElement(C, {
                          image:
                            "springcleaning2025/cornucopia/vector_snapping.jpg",
                          additionalClassName: (0, g.A)(E().ImageShadowMedium),
                        }),
                        c.createElement(
                          "div",
                          { className: E().TextBlock },
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockTitle,
                                E().LabelFont,
                                E().LabelExtraLarge,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_vector_targeting",
                            ),
                          ),
                          c.createElement(
                            "p",
                            {
                              className: (0, g.A)(
                                E().BlockDescription,
                                E().BodyFont,
                                E().BodyLarge,
                                E().LightGrayText,
                              ),
                            },
                            (0, l.Wn)(
                              "#springcleaning2025_website_features_entry_vector_targeting_desc",
                            ),
                          ),
                        ),
                      ),
                    ),
                    c.createElement(
                      "div",
                      { className: E().InnerContainer },
                      c.createElement(
                        "p",
                        {
                          className: (0, g.A)(
                            E().BlockTitle,
                            E().TitleFont,
                            E().TitleMedium,
                          ),
                        },
                        (0, l.Wn)(
                          "#springcleaning2025_website_features_entry_everything_else_else",
                        ),
                      ),
                      c.createElement(
                        "div",
                        { className: (0, g.A)(E().BugfixListContainer) },
                        c.createElement(
                          "p",
                          {
                            className: (0, g.A)(
                              E().BugFixCategoryTitle,
                              E().LabelFont,
                              E().LabelMedium,
                            ),
                          },
                          (0, l.Wn)(
                            "#springcleaning2025_website_bugfix_general",
                          ),
                        ),
                        c.createElement(
                          "div",
                          { className: (0, g.A)(E().BugfixListColumn) },
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix1",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix2",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix3",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix4",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix5",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix6",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix7",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix8",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix9",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix10",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix11",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix12",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix13",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix14",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix15",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix16",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix17",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix18",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix19",
                          }),
                          c.createElement(H, {
                            description:
                              "#springcleaning2025_website_bugfixScan",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix20",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix21",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix22",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix23",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix24",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix25",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix26",
                          }),
                        ),
                      ),
                    ),
                    c.createElement(
                      "div",
                      { className: E().InnerContainer },
                      c.createElement(
                        "p",
                        {
                          className: (0, g.A)(
                            E().BlockTitle,
                            E().TitleFont,
                            E().TitleMedium,
                          ),
                        },
                        (0, l.Wn)("Bug fixes"),
                      ),
                      c.createElement(
                        "div",
                        { className: (0, g.A)(E().BugfixListContainer) },
                        c.createElement(
                          "p",
                          {
                            className: (0, g.A)(
                              E().BugFixCategoryTitle,
                              E().LabelFont,
                              E().LabelMedium,
                            ),
                          },
                          (0, l.Wn)(
                            "#springcleaning2025_website_bugfix_general",
                          ),
                        ),
                        c.createElement(
                          "div",
                          { className: (0, g.A)(E().BugfixListColumn) },
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix31",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix32",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix33",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix54",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix55",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix56",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix58",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix59",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix60",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix61",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix62",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix63",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix64",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix65",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix66",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix67",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix68",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix69",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix70",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix71",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix72",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix73",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix74",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix75",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix76",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix77",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix78",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix79",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix80",
                          }),
                        ),
                      ),
                      c.createElement(
                        "div",
                        { className: (0, g.A)(E().BugfixListContainer) },
                        c.createElement(
                          "p",
                          {
                            className: (0, g.A)(
                              E().BugFixCategoryTitle,
                              E().LabelFont,
                              E().LabelMedium,
                            ),
                          },
                          (0, l.Wn)(
                            "#springcleaning2025_website_bugfix_plus_relics",
                          ),
                        ),
                        c.createElement(
                          "div",
                          { className: (0, g.A)(E().BugfixListColumn) },
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix35",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix36",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix37",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix38",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix39",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix40",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix41",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix42",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix43",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix44",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix45",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix46",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix47",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix48",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix49",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix50",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix51",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix52",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix53",
                          }),
                        ),
                      ),
                      c.createElement(
                        "div",
                        { className: (0, g.A)(E().BugfixListContainer) },
                        c.createElement(
                          "p",
                          {
                            className: (0, g.A)(
                              E().BugFixCategoryTitle,
                              E().LabelFont,
                              E().LabelMedium,
                            ),
                          },
                          (0, l.Wn)(
                            "#springcleaning2025_website_bugfix_tooltips",
                          ),
                        ),
                        c.createElement(
                          "div",
                          { className: (0, g.A)(E().BugfixListColumn) },
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix82",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix83",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix84",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix85",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix86",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix87",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix88",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix89",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix90",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix91",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix92",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix93",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix94",
                          }),
                          c.createElement(H, {
                            description: "#springcleaning2025_website_bugfix95",
                          }),
                        ),
                      ),
                    ),
                  ),
                  c.createElement(C, {
                    image:
                      "springcleaning2025/backgrounds/spring_leaves_footer.png",
                    additionalClassName: (0, g.A)(E().FooterImage),
                  }),
                ),
                G(),
                " ",
                c.createElement(
                  "div",
                  {
                    id: "TreasureSection",
                    className: (0, g.A)(
                      E().WebsiteSection,
                      E().TreasureSection,
                    ),
                  },
                  c.createElement("div", { className: E().TreasureBackground }),
                  c.createElement("div", { className: E().TopShadow }),
                  c.createElement(
                    "div",
                    { className: E().WebsiteSectionInner },
                    c.createElement(
                      "div",
                      { className: E().WebsiteSectionHeader },
                      c.createElement(
                        "p",
                        {
                          className: (0, g.A)(
                            E().SectionSubHeaderLabel,
                            E().LabelFont,
                            E().LabelMedium,
                            E().GrayText,
                          ),
                        },
                        (0, l.Wn)(
                          "#springcleaning2025_website_treasures_label",
                        ),
                      ),
                      c.createElement(
                        "h2",
                        {
                          className: (0, g.A)(
                            E().SectionHeaderLabel,
                            E().TitleFont,
                            E().TitleExtraLarge,
                          ),
                        },
                        (0, l.Wn)(
                          "#springcleaning2025_website_treasures_subheader",
                        ),
                      ),
                      c.createElement(
                        "p",
                        {
                          className: (0, g.A)(
                            E().WebsiteDescription,
                            E().DisplayFont,
                            E().DisplaySmall,
                            E().LightGrayText,
                          ),
                        },
                        (0, l.Wn)(
                          "#springcleaning2025_website_treasures_subheader_desc",
                        ),
                      ),
                    ),
                    c.createElement(
                      N.gi,
                      {
                        className: E().TreasureCarousel,
                        naturalSlideWidth: 600,
                        naturalSlideHeight: 960,
                        totalSlides: 11,
                        currentSlide: 7,
                        infinite: !0,
                        touchEnabled: !0,
                        dragEnabled: !1,
                      },
                      c.createElement(
                        N.Ap,
                        { className: E().TreasureSlider },
                        c.createElement(
                          N.q7,
                          {
                            index: 0,
                            className: E().TreasureSlide,
                            innerClassName: E().TreasureInnerSlide,
                            classNameHidden: E().TreasureSlideHidden,
                          },
                          c.createElement(D, {
                            index: 0,
                            video: "set_spiritbreaker",
                            name: "#springcleaning2025_treasure_hero_spiritbreaker_set",
                            heroname:
                              "#springcleaning2025_treasure_hero_spiritbreaker",
                            autoplay: !1,
                            onSlideIn: (e, a) => {
                              this.setState({ treasureName: e, heroName: a });
                            },
                          }),
                        ),
                        c.createElement(
                          N.q7,
                          {
                            index: 1,
                            className: E().TreasureSlide,
                            innerClassName: E().TreasureInnerSlide,
                            classNameHidden: E().TreasureSlideHidden,
                          },
                          c.createElement(D, {
                            index: 1,
                            video: "set_antimage",
                            name: "#springcleaning2025_treasure_hero_antimage_set",
                            heroname:
                              "#springcleaning2025_treasure_hero_antimage",
                            autoplay: !1,
                            onSlideIn: (e, a) => {
                              this.setState({ treasureName: e, heroName: a });
                            },
                          }),
                        ),
                        c.createElement(
                          N.q7,
                          {
                            index: 2,
                            className: E().TreasureSlide,
                            innerClassName: E().TreasureInnerSlide,
                            classNameHidden: E().TreasureSlideHidden,
                          },
                          c.createElement(D, {
                            index: 2,
                            video: "set_luna",
                            name: "#springcleaning2025_treasure_hero_luna_set",
                            heroname: "#springcleaning2025_treasure_hero_luna",
                            autoplay: !1,
                            onSlideIn: (e, a) => {
                              this.setState({ treasureName: e, heroName: a });
                            },
                          }),
                        ),
                        c.createElement(
                          N.q7,
                          {
                            index: 3,
                            className: E().TreasureSlide,
                            innerClassName: E().TreasureInnerSlide,
                            classNameHidden: E().TreasureSlideHidden,
                          },
                          c.createElement(D, {
                            index: 3,
                            video: "set_invoker",
                            name: "#springcleaning2025_treasure_hero_invoker_set",
                            heroname:
                              "#springcleaning2025_treasure_hero_invoker",
                            autoplay: !1,
                            onSlideIn: (e, a) => {
                              this.setState({ treasureName: e, heroName: a });
                            },
                          }),
                        ),
                        c.createElement(
                          N.q7,
                          {
                            index: 4,
                            className: E().TreasureSlide,
                            innerClassName: E().TreasureInnerSlide,
                            classNameHidden: E().TreasureSlideHidden,
                          },
                          c.createElement(D, {
                            index: 4,
                            video: "set_necrophos",
                            name: "#springcleaning2025_treasure_hero_necrophos_set",
                            heroname:
                              "#springcleaning2025_treasure_hero_necrophos",
                            autoplay: !1,
                            onSlideIn: (e, a) => {
                              this.setState({ treasureName: e, heroName: a });
                            },
                          }),
                        ),
                        c.createElement(
                          N.q7,
                          {
                            index: 5,
                            className: E().TreasureSlide,
                            innerClassName: E().TreasureInnerSlide,
                            classNameHidden: E().TreasureSlideHidden,
                          },
                          c.createElement(D, {
                            index: 5,
                            video: "set_huskar",
                            name: "#springcleaning2025_treasure_hero_huskar_set",
                            heroname:
                              "#springcleaning2025_treasure_hero_huskar",
                            autoplay: !1,
                            onSlideIn: (e, a) => {
                              this.setState({ treasureName: e, heroName: a });
                            },
                          }),
                        ),
                        c.createElement(
                          N.q7,
                          {
                            index: 6,
                            className: E().TreasureSlide,
                            innerClassName: E().TreasureInnerSlide,
                            classNameHidden: E().TreasureSlideHidden,
                          },
                          c.createElement(D, {
                            index: 6,
                            video: "set_axe",
                            name: "#springcleaning2025_treasure_hero_axe_set",
                            heroname: "#springcleaning2025_treasure_hero_axe",
                            autoplay: !1,
                            onSlideIn: (e, a) => {
                              this.setState({ treasureName: e, heroName: a });
                            },
                          }),
                        ),
                        c.createElement(
                          N.q7,
                          {
                            index: 7,
                            className: E().TreasureSlide,
                            innerClassName: E().TreasureInnerSlide,
                            classNameHidden: E().TreasureSlideHidden,
                          },
                          c.createElement(D, {
                            index: 7,
                            video: "set_pudge",
                            name: "#springcleaning2025_treasure_hero_pudge_set",
                            heroname: "#springcleaning2025_treasure_hero_pudge",
                            autoplay: !0,
                            onSlideIn: (e, a) => {
                              this.setState({ treasureName: e, heroName: a });
                            },
                          }),
                        ),
                        c.createElement(
                          N.q7,
                          {
                            index: 8,
                            className: E().TreasureSlide,
                            innerClassName: E().TreasureInnerSlide,
                            classNameHidden: E().TreasureSlideHidden,
                          },
                          c.createElement(D, {
                            index: 8,
                            video: "set_earthshaker",
                            name: "#springcleaning2025_treasure_hero_earthshaker_set",
                            heroname:
                              "#springcleaning2025_treasure_hero_earthshaker",
                            autoplay: !1,
                            onSlideIn: (e, a) => {
                              this.setState({ treasureName: e, heroName: a });
                            },
                          }),
                        ),
                        c.createElement(
                          N.q7,
                          {
                            index: 9,
                            className: E().TreasureSlide,
                            innerClassName: E().TreasureInnerSlide,
                            classNameHidden: E().TreasureSlideHidden,
                          },
                          c.createElement(D, {
                            index: 9,
                            video: "set_sandking",
                            name: "#springcleaning2025_treasure_hero_sandking_set",
                            heroname:
                              "#springcleaning2025_treasure_hero_sandking",
                            autoplay: !1,
                            onSlideIn: (e, a) => {
                              this.setState({ treasureName: e, heroName: a });
                            },
                          }),
                        ),
                        c.createElement(
                          N.q7,
                          {
                            index: 10,
                            className: E().TreasureSlide,
                            innerClassName: E().TreasureInnerSlide,
                            classNameHidden: E().TreasureSlideHidden,
                          },
                          c.createElement(D, {
                            index: 10,
                            video: "set_lion",
                            name: "#springcleaning2025_treasure_hero_lion_set",
                            heroname: "#springcleaning2025_treasure_hero_lion",
                            autoplay: !1,
                            onSlideIn: (e, a) => {
                              this.setState({ treasureName: e, heroName: a });
                            },
                          }),
                        ),
                      ),
                      c.createElement(
                        "p",
                        {
                          className: (0, g.A)(
                            E().HeroName,
                            E().LabelFont,
                            E().LabelSmall,
                          ),
                        },
                        (0, l.Wn)(this.state.heroName),
                      ),
                      c.createElement(
                        "p",
                        {
                          className: (0, g.A)(
                            E().TreasureName,
                            E().DisplayFont,
                            E().DisplaySmall,
                          ),
                        },
                        (0, l.Wn)(this.state.treasureName),
                      ),
                      c.createElement(
                        "div",
                        { className: E().CarouselDots },
                        c.createElement(
                          N._X,
                          {
                            className: (0, g.A)(
                              E().TreasurePaginationButton,
                              E().Prev,
                            ),
                          },
                          c.createElement("div", { className: E().PrevArrow }),
                        ),
                        c.createElement(
                          N.cL,
                          { className: E().TreasureSelector, slide: 0 },
                          c.createElement("div", null),
                        ),
                        c.createElement(
                          N.cL,
                          { className: E().TreasureSelector, slide: 1 },
                          c.createElement("div", null),
                        ),
                        c.createElement(
                          N.cL,
                          { className: E().TreasureSelector, slide: 2 },
                          c.createElement("div", null),
                        ),
                        c.createElement(
                          N.cL,
                          { className: E().TreasureSelector, slide: 3 },
                          c.createElement("div", null),
                        ),
                        c.createElement(
                          N.cL,
                          { className: E().TreasureSelector, slide: 4 },
                          c.createElement("div", null),
                        ),
                        c.createElement(
                          N.cL,
                          { className: E().TreasureSelector, slide: 5 },
                          c.createElement("div", null),
                        ),
                        c.createElement(
                          N.cL,
                          { className: E().TreasureSelector, slide: 6 },
                          c.createElement("div", null),
                        ),
                        c.createElement(
                          N.cL,
                          { className: E().TreasureSelector, slide: 7 },
                          c.createElement("div", null),
                        ),
                        c.createElement(
                          N.cL,
                          { className: E().TreasureSelector, slide: 8 },
                          c.createElement("div", null),
                        ),
                        c.createElement(
                          N.cL,
                          { className: E().TreasureSelector, slide: 9 },
                          c.createElement("div", null),
                        ),
                        c.createElement(
                          N.cL,
                          { className: E().TreasureSelector, slide: 10 },
                          c.createElement("div", null),
                        ),
                        c.createElement(
                          N.CC,
                          {
                            className: (0, g.A)(
                              E().TreasurePaginationButton,
                              E().Next,
                            ),
                          },
                          c.createElement("div", { className: E().NextArrow }),
                        ),
                      ),
                    ),
                  ),
                ),
                G(),
                " ",
                c.createElement(
                  "div",
                  {
                    id: "GameplayUpdateContainer",
                    className: (0, g.A)(
                      E().WebsiteSection,
                      E().GameplayUpdateContainer,
                    ),
                  },
                  c.createElement("div", { className: E().TopShadow }),
                  c.createElement(
                    "div",
                    { className: E().WebsiteSectionInner },
                    c.createElement(
                      "div",
                      { className: E().WebsiteSectionHeader },
                      c.createElement(
                        "h2",
                        {
                          className: (0, g.A)(
                            E().SectionHeaderLabel,
                            E().TitleFont,
                            E().TitleExtraLarge,
                          ),
                        },
                        (0, l.Wn)(
                          "#springcleaning2025_website_section_header_patch",
                        ),
                      ),
                      c.createElement(
                        "p",
                        {
                          className: (0, g.A)(
                            E().DisplayFont,
                            E().DisplayMedium,
                          ),
                        },
                        (0, l.Wn)(
                          "#springcleaning2025_website_gameplay_introduction",
                        ),
                      ),
                    ),
                    M(),
                    c.createElement(
                      "div",
                      { className: (0, g.A)(E().WebsiteSectionHeader) },
                      c.createElement(
                        "h2",
                        {
                          className: (0, g.A)(
                            E().SectionHeaderLabel,
                            E().TitleFont,
                            E().TitleLarge,
                          ),
                        },
                        (0, l.Wn)(
                          "#springcleaning2025_website_gameplay_hero_highlights",
                        ),
                      ),
                      c.createElement(
                        "p",
                        {
                          className: (0, g.A)(
                            E().SectionDescriptionLabel,
                            E().DisplayFont,
                            E().DisplaySmall,
                            E().LightGrayText,
                          ),
                        },
                        (0, l.Wn)(
                          "#springcleaning2025_website_gameplay_hero_highlights_introduction",
                        ),
                      ),
                    ),
                    c.createElement(
                      "div",
                      {
                        className: (0, g.A)(
                          E().Grid_3,
                          E().HeroHighlightsContainer,
                        ),
                      },
                      c.createElement(S, {
                        nHeroID: 51,
                        strFacetName: "Chainmeal",
                        strScale: "0.95",
                        strTranslateX: "-3%",
                        strTranslateY: "2%",
                        patchnotes: e,
                      }),
                      c.createElement(S, {
                        nHeroID: 53,
                        strFacetName: "Nature's Profit",
                        strScale: "1.2",
                        strTranslateX: "6%",
                        strTranslateY: "-7%",
                        patchnotes: e,
                      }),
                      c.createElement(S, {
                        nHeroID: 114,
                        strFacetName: "Changing of the Guard",
                        strScale: "1.2",
                        strTranslateX: "-13%",
                        strTranslateY: "1%",
                        patchnotes: e,
                      }),
                    ),
                    c.createElement(C, {
                      image: "springcleaning2025/DOTA_7_39_hero_highlights.jpg",
                      video: "springcleaning2025/DOTA_7_39_hero_highlights.mp4",
                      additionalClassName: (0, g.A)(
                        E().FullWidthImage,
                        E().ImageShadowMedium,
                      ),
                    }),
                    M(),
                    c.createElement(
                      "div",
                      { className: (0, g.A)(E().WebsiteSectionHeader) },
                      c.createElement(
                        "h2",
                        {
                          className: (0, g.A)(
                            E().SectionHeaderLabel,
                            E().TitleFont,
                            E().TitleLarge,
                          ),
                        },
                        (0, l.Wn)(
                          "#springcleaning2025_website_gameplay_terrain_highlights",
                        ),
                      ),
                      c.createElement(
                        "p",
                        {
                          className: (0, g.A)(
                            E().SectionDescriptionLabel,
                            E().DisplayFont,
                            E().DisplaySmall,
                            E().LightGrayText,
                          ),
                        },
                        (0, l.Wn)(
                          "#springcleaning2025_website_gameplay_terrain_highlights_introduction",
                        ),
                      ),
                    ),
                    c.createElement(
                      "div",
                      { className: (0, g.A)(E().ComparisonContainer) },
                      c.createElement(
                        w.U,
                        { labels: ["1", "2", "3", "4", "5", "6"] },
                        c.createElement(x.EW, {
                          itemOne: c.createElement(w.v, {
                            is_new: !0,
                            image: "springcleaning2025/terrain/old/01.jpg",
                          }),
                          itemTwo: c.createElement(w.v, {
                            is_new: !1,
                            image: "springcleaning2025/terrain/new/01.jpg",
                          }),
                        }),
                        c.createElement(x.EW, {
                          itemOne: c.createElement(w.v, {
                            is_new: !0,
                            image: "springcleaning2025/terrain/old/02.jpg",
                          }),
                          itemTwo: c.createElement(w.v, {
                            is_new: !1,
                            image: "springcleaning2025/terrain/new/02.jpg",
                          }),
                        }),
                        c.createElement(x.EW, {
                          itemOne: c.createElement(w.v, {
                            is_new: !0,
                            image: "springcleaning2025/terrain/old/03.jpg",
                          }),
                          itemTwo: c.createElement(w.v, {
                            is_new: !1,
                            image: "springcleaning2025/terrain/new/03.jpg",
                          }),
                        }),
                        c.createElement(x.EW, {
                          itemOne: c.createElement(w.v, {
                            is_new: !0,
                            image: "springcleaning2025/terrain/old/04.jpg",
                          }),
                          itemTwo: c.createElement(w.v, {
                            is_new: !1,
                            image: "springcleaning2025/terrain/new/04.jpg",
                          }),
                        }),
                        c.createElement(x.EW, {
                          itemOne: c.createElement(w.v, {
                            is_new: !0,
                            image: "springcleaning2025/terrain/old/05.jpg",
                          }),
                          itemTwo: c.createElement(w.v, {
                            is_new: !1,
                            image: "springcleaning2025/terrain/new/05.jpg",
                          }),
                        }),
                        c.createElement(x.EW, {
                          itemOne: c.createElement(w.v, {
                            is_new: !0,
                            image: "springcleaning2025/terrain/old/06.jpg",
                          }),
                          itemTwo: c.createElement(w.v, {
                            is_new: !1,
                            image: "springcleaning2025/terrain/new/06.jpg",
                          }),
                        }),
                      ),
                    ),
                    M(),
                    c.createElement(
                      "div",
                      { className: (0, g.A)(E().WebsiteSectionHeader) },
                      c.createElement(
                        "h2",
                        {
                          className: (0, g.A)(
                            E().SectionHeaderLabel,
                            E().TitleFont,
                            E().TitleLarge,
                          ),
                        },
                        (0, l.Wn)(
                          "#springcleaning2025_website_gameplay_neutral_highlights",
                        ),
                      ),
                      c.createElement(
                        "p",
                        {
                          className: (0, g.A)(
                            E().SectionDescriptionLabel,
                            E().DisplayFont,
                            E().DisplaySmall,
                            E().LightGrayText,
                          ),
                        },
                        (0, l.Wn)(
                          "#springcleaning2025_website_gameplay_neutral_highlights_introduction",
                        ),
                      ),
                    ),
                    c.createElement(
                      "div",
                      { className: E().GameItemsContainer },
                      c.createElement(W, { name: "item_dormant_curio" }),
                      c.createElement(W, { name: "item_kobold_cup" }),
                      c.createElement(W, { name: "item_sisters_shroud" }),
                      c.createElement(W, { name: "item_jidi_pollen_bag" }),
                      c.createElement(W, { name: "item_dezun_bloodrite" }),
                      c.createElement(W, { name: "item_giant_maul" }),
                      c.createElement(W, { name: "item_outworld_staff" }),
                      c.createElement(W, { name: "item_divine_regalia" }),
                    ),
                    M(),
                    c.createElement(
                      "div",
                      {
                        className: (0, g.A)(
                          E().WebsiteSectionHeader,
                          E().FeatureHeader,
                        ),
                      },
                      c.createElement(
                        "h2",
                        {
                          className: (0, g.A)(
                            E().SectionHeaderLabel,
                            E().TitleFont,
                            E().TitleLarge,
                          ),
                        },
                        (0, l.Wn)(
                          "#springcleaning2025_website_gameplay_patchnotes",
                        ),
                      ),
                    ),
                    c.createElement(
                      "div",
                      { className: E().PatchnotesContainer },
                      c.createElement(b.fs, {
                        patchnotes: e?.general_notes,
                        headerClassName: E().PatchNotesHeaderLabel,
                        notesListClassName: E().PatchNotesList,
                      }),
                      c.createElement(b.wL, {
                        patchnotes: e?.neutral_creeps,
                        headerClassName: E().PatchNotesHeaderLabel,
                        notesListClassName: E().PatchNotesList,
                      }),
                      c.createElement(b.ZV, {
                        patchnotes: e?.items,
                        headerClassName: E().PatchNotesHeaderLabel,
                        notesListClassName: E().PatchNotesList,
                      }),
                      c.createElement(b.ZV, {
                        patchnotes: e?.neutral_items,
                        is_neutrals: !0,
                        headerClassName: E().PatchNotesHeaderLabel,
                        notesListClassName: E().PatchNotesList,
                      }),
                      c.createElement(b.ob, {
                        patchnotes: e?.heroes,
                        headerClassName: E().PatchNotesHeaderLabel,
                        notesListClassName: E().PatchNotesList,
                      }),
                    ),
                  ),
                ),
                c.createElement(p.K, null),
              ),
            )
          );
        }
      };
      R = (0, n.Cg)([s.PA], R);
      const P = R;
    },
  },
]);
